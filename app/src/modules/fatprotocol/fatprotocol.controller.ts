...
import { CreateProtocolDto, UpdateProtocolDto, ListQueryDto } from './fatprotocol.dto';
...
export class FatProtocolController {
  constructor(private readonly svc: FatProtocolService) {}

  create = async (req: Request, res: Response): Promise<void> => {
    res.status(201).json(await this.svc.create(ctxOf(req), valid<CreateProtocolDto>(req)));
  };
  ...
  update = async (req: Request, res: Response): Promise<void> => {
    res.json(await this.svc.update(ctxOf(req), idOf(req), valid<UpdateProtocolDto>(req)));
  };
