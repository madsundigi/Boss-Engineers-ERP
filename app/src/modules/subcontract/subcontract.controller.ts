import { Request, Response } from 'express';
import { SubcontractService } from './subcontract.service';
import { valid } from '../../common/validate';
import { Errors } from '../../common/http-error';
import { RequestContext } from '../../common/request-context';
import {
  CreateSubcontractDto, UpdateSubcontractDto, IssueDto, ReceiveDto, CancelDto, VersionDto, ListQueryDto,
} from './subcontract.dto';

function ctxOf(req: Request): RequestContext {
  if (!req.context) throw Errors.unauthorized();
  return req.context;
}
function idOf(req: Request): number {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) throw Errors.badRequest('Invalid id');
  return id;
}

export class SubcontractController {
  constructor(private readonly service: SubcontractService) {}

  list = async (req: Request, res: Response) => {
    res.json(await this.service.list(ctxOf(req), valid<ListQueryDto>(req, 'query')));
  };

  create = async (req: Request, res: Response) => {
    const created = await this.service.create(ctxOf(req), valid<CreateSubcontractDto>(req));
    res.status(201).json(created);
  };

  getById = async (req: Request, res: Response) => {
    res.json(await this.service.getById(ctxOf(req), idOf(req)));
  };

  update = async (req: Request, res: Response) => {
    res.json(await this.service.update(ctxOf(req), idOf(req), valid<UpdateSubcontractDto>(req)));
  };

  issue = async (req: Request, res: Response) => {
    res.json(await this.service.issueMaterial(ctxOf(req), idOf(req), valid<IssueDto>(req)));
  };

  receive = async (req: Request, res: Response) => {
    res.json(await this.service.receiveGoods(ctxOf(req), idOf(req), valid<ReceiveDto>(req)));
  };

  close = async (req: Request, res: Response) => {
    res.json(await this.service.close(ctxOf(req), idOf(req), valid<VersionDto>(req).rowVersion));
  };

  cancel = async (req: Request, res: Response) => {
    res.json(await this.service.cancel(ctxOf(req), idOf(req), valid<CancelDto>(req)));
  };

  remove = async (req: Request, res: Response) => {
    await this.service.delete(ctxOf(req), idOf(req));
    res.status(204).send();
  };

  exportCsv = async (req: Request, res: Response) => {
    const csv = await this.service.exportCsv(ctxOf(req), valid<ListQueryDto>(req, 'query'));
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename="subcontracts.csv"');
    res.send(csv);
  };
}
