...
export class FatController {
  constructor(private readonly service: FatService) {}
  ...
  create = async (req: Request, res: Response) => {
    const created = await this.service.create(ctxOf(req), valid<CreateFatDto>(req));
  ...
  update = async (req: Request, res: Response) => {
    res.json(await this.service.update(ctxOf(req), idOf(req), valid<UpdateFatDto>(req)));
  };

  recordResult = async (req: Request, res: Response) => {
    res.json(await this.service.recordResult(ctxOf(req), idOf(req), valid<RecordResultDto>(req)));
  };
  ...
  approve = async (req: Request, res: Response) => {
    res.json(await this.service.approve(ctxOf(req), idOf(req), valid<ApproveDto>(req)));
  };
  ...
  exportCsv = async (req: Request, res: Response) => {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="fat.csv"');
    res.send(csv);
  };
}
