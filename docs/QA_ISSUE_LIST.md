...
> **Remediation status (2026-06-06)** — hardening + transactional outbox (69 tests green). **Fixed:** RC-01 (double-convert unique index), RC-02 (send optimistic lock), MV-02 (bu/company composite FK), MV-03 (decimal money), MV-06 (array bound), PI-03 (SoD self-approval), **BW-02 (email-before-commit — `send` now marks SENT + emits a `quotation.sent` event atomically; the relay emails the PDF after commit with retry/backoff/dead-letter).** **Foundation in place:** CC-01/BW-01 — transactional outbox added (`mdm.outbox_event` + `OutboxRelay`); `send` uses it; the convert/won/lost enquiry sync still runs synchronously and should migrate to outbox events next. **Mitigated:** PI-01 (RLS policies, erp_app role), MV-01/PI-04 (JWT verify added; gateway still required in prod), PI-02 (doa_rule table; enforcement pending). **Still open:** BW-04 (semantic audit events), DL-02 (idempotency keys), DL-01 (backup/DR), RC-03 (inventory reservation lock — pre-M06), BW-01 (migrate convert/won/lost sync to the outbox).
